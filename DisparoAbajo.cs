...
public class DisparoAbajo : MonoBehaviour
{
    ...
    void disparo()
    {
        ...
        RaycastHit2D hit = Physics2D.Raycast(transform.position, Vector2.down, distanciaDisparo); //Creamos el metodo
        if (hit.collider != null) //Si el rayo hace contacto con algo pasa lo siguiente
        {
            if (hit.collider.CompareTag("Player"))//Si es el jugador
            {
                ...
                {
                    efecto.Play();
                    animacion.Play("ataque");//Activa la animacion
                    tiempoActual = tiempoAtaque;//Lo iguala para no dispara mas hasta que vuelva a 0
                    Instantiate(aguijon, transform.position, transform.rotation);//Crea el objeto todas las veces que haga el contacto
                }
            }
        }
    }
