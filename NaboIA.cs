...
public class NaboIA : MonoBehaviour
{
    //ATRIBUTOS
    Animator animacion;
    ...
    IEnumerator chekeo()//Chekeara la posicion para rotar el eje x con una rutina y activara las animaciones que hagan falta
    {
        ...
        if (transform.position.x > posicionActual.x)
        {
            ...
            gameObject.transform.GetChild(2).gameObject.SetActive(false);//Ira desactivando en el sentido que mire el gameobject que invoca las balas
        ...
        else if (transform.position.x < posicionActual.x)
        {
            imagen.flipX = false;
            animacion.SetBool("Para", false);
            gameObject.transform.GetChild(2).gameObject.SetActive(true);//Igual
